export const allCelulares = [
	{
		brand: 'Samsung',
		colors: [{ color: '#000000', color_name: 'ดำ' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['แรม 8GB', 'รองรับ 2 ซิม', 'กล้องหลัง 48MP'],
		id: '4f53b2ea-2a40-4ec7-a6d0-b7c95888ff',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'Samsung A55 5G',
		price: 449.99,
		slug: 'samsung-a55-5g',
		variants: [
			{
				color: '#000000',
				color_name: 'ดำ',
				id: 'a4e814b7-fe26-4e08-8a6f-13a39da43e52',
				price: 449.99,
				stock: 10,
				storage: '256GB',
			},
		],
	},
	{
		brand: 'Apple',
		colors: [{ color: '#FFFFFF', color_name: 'ขาว' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['ความจุ 128GB', 'Face ID', 'กล้องหน้า 12MP'],
		id: '1b3c2de7-3a10-4e9b-b8e7-b5c8a1234563',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'iPhone 13',
		price: 999.99,
		slug: 'iphone-13',
		variants: [
			{
				color: '#FFFFFF',
				color_name: 'ขาว',
				id: 'a3f1a5b7-fe26-4e08-8b7e-16a45cd43f11',
				price: 999.99,
				stock: 5,
				storage: '128GB',
			},
		],
	},
	{
		brand: 'Xiaomi',
		colors: [{ color: '#FF0000', color_name: 'แดง' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['แรม 6GB', 'แบตเตอรี่ 5000mAh', 'หน้าจอ 90Hz'],
		id: 'e5a1d2be-5b60-4f3e-b8d1-a3c98c123abd',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'Xiaomi Redmi Note 10',
		price: 299.99,
		slug: 'xiaomi-redmi-note-10',
		variants: [
			{
				color: '#FF0000',
				color_name: 'แดง',
				id: 'b2c3d4f5-fe26-4e08-8e0f-17a58bd43c12',
				price: 299.99,
				stock: 15,
				storage: '128GB',
			},
		],
	},
	{
		brand: 'Huawei',
		colors: [{ color: '#0000FF', color_name: 'น้ำเงิน' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['แรม 128GB', 'จอ OLED', 'ชาร์จเร็ว'],
		id: 'f8c3b4a9-4d3a-4f6b-a8d1-b3c78c143cde',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'Huawei P40 Pro',
		price: 799.99,
		slug: 'huawei-p40-pro',
		variants: [
			{
				color: '#0000FF',
				color_name: 'น้ำเงิน',
				id: 'c1d2e3f4-fe26-4e08-8f1e-18a69bd45d13',
				price: 799.99,
				stock: 8,
				storage: '256GB',
			},
		],
	},
	{
		brand: 'Realme',
		colors: [{ color: '#FFA500', color_name: 'ส้ม' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['แรม 6GB', 'Snapdragon 870', 'กล้อง 64MP'],
		id: 'c1f7e6d8-7c5d-4e8b-a9f0-b3d88b987abc',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'Realme GT',
		price: 499.99,
		slug: 'realme-gt',
		variants: [
			{
				color: '#FFA500',
				color_name: 'ส้ม',
				id: 'j8k9l0m1-fe26-4e08-9j8i-25a38gf12k20',
				price: 499.99,
				stock: 10,
				storage: '128GB',
			},
		],
	},
	{
		brand: 'Honor',
		colors: [{ color: '#0000FF', color_name: 'น้ำเงิน' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['แรม 8GB', 'จอ AMOLED', 'กล้อง 108MP'],
		id: 'f1d2e3b4-6a4c-4f5e-b9e2-a3b18c111bcd',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'Honor 50',
		price: 599.99,
		slug: 'honor-50',
		variants: [
			{
				color: '#0000FF',
				color_name: 'น้ำเงิน',
				id: 'k1l2m3n4-fe26-4e08-9a1b-26a49dh12i21',
				price: 599.99,
				stock: 10,
				storage: '256GB',
			},
		],
	},
	{
		brand: 'Realme',
		colors: [{ color: '#FFA500', color_name: 'ส้ม' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['แรม 6GB', 'Snapdragon 870', 'กล้อง 64MP'],
		id: 'c9f7e6d8-7c5d-4e8b-a9f0-b3d88b987abc',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'Realme GT',
		price: 499.99,
		slug: 'realme-gt',
		variants: [
			{
				color: '#FFA500',
				color_name: 'ส้ม',
				id: 'j8k9l0m1-fe26-4e08-9j8i-25a38gf12k20',
				price: 499.99,
				stock: 10,
				storage: '128GB',
			},
		],
	},
	{
		brand: 'Huawei',
		colors: [{ color: '#800080', color_name: 'ม่วง' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['แรม 128GB', 'จอ OLED', 'ชาร์จเร็ว'],
		id: 'f8c3b4a9-4d3a-4f6b-a8d1-b3c78c123cde',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'Huawei P40 Pro',
		price: 799.99,
		slug: 'huawei-p40-pro',
		variants: [
			{
				color: '#800080',
				color_name: 'ม่วง',
				id: 'i1j2k3l4-fe26-4e08-9g5f-27a50gj13m22',
				price: 799.99,
				stock: 8,
				storage: '256GB',
			},
		],
	},
	{
		brand: 'Apple',
		colors: [{ color: '#FFFFFF', color_name: 'ขาว' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['ความจุ 128GB', 'Face ID', 'กล้องหน้า 12MP'],
		id: '1b3c2de7-3a10-4e9b-b8e7-b5c8a1234560',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'iPhone 13',
		price: 999.99,
		slug: 'iphone-13',
		variants: [
			{
				color: '#FFFFFF',
				color_name: 'ขาว',
				id: 'a3f1a5b7-fe26-4e08-8b7e-16a45cd43f11',
				price: 999.99,
				stock: 5,
				storage: '128GB',
			},
		],
	},
	{
		brand: 'Samsung',
		colors: [{ color: '#FF0000', color_name: 'แดง' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['แรม 6GB', 'จอ Super AMOLED', 'กล้อง 64MP'],
		id: 'f1e2d3b4-7a5c-4f5b-b9e1-a3d18c321bcd',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'Samsung Galaxy A72',
		price: 699.99,
		slug: 'samsung-galaxy-a72',
		variants: [
			{
				color: '#FF0000',
				color_name: 'แดง',
				id: 'l2m3n4o5-fe26-4e08-9a2b-28a61dh14k23',
				price: 699.99,
				stock: 12,
				storage: '256GB',
			},
		],
	},
	{
		brand: 'Xiaomi',
		colors: [{ color: '#00FF00', color_name: 'เขียว' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['แรม 8GB', 'แบตเตอรี่ 5000mAh', 'หน้าจอ 90Hz'],
		id: 'e5a1d2be-5b60-4f3e-b8d1-a3c98c123abc',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'Xiaomi Redmi Note 10',
		price: 299.99,
		slug: 'xiaomi-redmi-note-10',
		variants: [
			{
				color: '#00FF00',
				color_name: 'เขียว',
				id: 'k4l5m6n7-fe26-4e08-9c2d-29a72ef15l24',
				price: 299.99,
				stock: 15,
				storage: '128GB',
			},
		],
	},
];

export const recentCelulares = [
	{
		brand: 'Samsung',
		colors: [{ color: '#000000', color_name: 'ดำ' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['แรม 8GB', 'รองรับ 2 ซิม', 'กล้องหลัง 48MP'],
		id: '4f53b2ea-2a40-4ec7-a6d0-b7c95888ff',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'Samsung A55 5G',
		price: 449.99,
		slug: 'samsung-a55-5g',
		variants: [
			{
				color: '#000000',
				color_name: 'ดำ',
				id: 'a4e814b7-fe26-4e08-8a6f-13a39da43e52',
				price: 449.99,
				stock: 10,
				storage: '256GB',
			},
		],
	},
	{
		brand: 'Apple',
		colors: [{ color: '#FFFFFF', color_name: 'ขาว' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['ความจุ 128GB', 'Face ID', 'กล้องหน้า 12MP'],
		id: '1b3c2de7-3a10-4e9b-b8e7-b5c8a1234167',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'iPhone 13',
		price: 999.99,
		slug: 'iphone-13',
		variants: [
			{
				color: '#FFFFFF',
				color_name: 'ขาว',
				id: 'a3f1a5b7-fe26-4e08-8b7e-16a45cd43f11',
				price: 999.99,
				stock: 5,
				storage: '128GB',
			},
		],
	},
	{
		brand: 'Xiaomi',
		colors: [{ color: '#FF0000', color_name: 'แดง' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['แรม 6GB', 'แบตเตอรี่ 5000mAh', 'หน้าจอ 90Hz'],
		id: 'e5a1d2be-5b60-4f3e-b8d1-a3c98c123abe',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'Xiaomi Redmi Note 10',
		price: 299.99,
		slug: 'xiaomi-redmi-note-10',
		variants: [
			{
				color: '#FF0000',
				color_name: 'แดง',
				id: 'b2c3d4f5-fe26-4e08-8e0f-17a58bd43c12',
				price: 299.99,
				stock: 15,
				storage: '128GB',
			},
		],
	},
	{
		brand: 'Huawei',
		colors: [{ color: '#0000FF', color_name: 'น้ำเงิน' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['แรม 128GB', 'จอ OLED', 'ชาร์จเร็ว'],
		id: 'f8c3b4a9-4d3a-4f6b-a8d1-b3c78c12wcde',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'Huawei P40 Pro',
		price: 799.99,
		slug: 'huawei-p40-pro',
		variants: [
			{
				color: '#0000FF',
				color_name: 'น้ำเงิน',
				id: 'c1d2e3f4-fe26-4e08-8f1e-18a69bd45d13',
				price: 799.99,
				stock: 8,
				storage: '256GB',
			},
		],
	},
];

export const popularCelulares = [
	{
		brand: 'Huawei',
		colors: [{ color: '#800080', color_name: 'ม่วง' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['แรม 128GB', 'จอ OLED', 'ชาร์จเร็ว'],
		id: 'f8c3b4a9-4d3a-4f6b-a8d1-b3c78c12fcde',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'Huawei P40 Pro',
		price: 799.99,
		slug: 'huawei-p40-pro',
		variants: [
			{
				color: '#800080',
				color_name: 'ม่วง',
				id: 'i1j2k3l4-fe26-4e08-9g5f-27a50gj13m22',
				price: 799.99,
				stock: 8,
				storage: '256GB',
			},
		],
	},
	{
		brand: 'Apple',
		colors: [{ color: '#FFFFFF', color_name: 'ขาว' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['ความจุ 128GB', 'Face ID', 'กล้องหน้า 12MP'],
		id: '1b3c2de7-3a10-4e9b-b8e7-b5c8a1234567',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'iPhone 13',
		price: 999.99,
		slug: 'iphone-13',
		variants: [
			{
				color: '#FFFFFF',
				color_name: 'ขาว',
				id: 'a3f1a5b7-fe26-4e08-8b7e-16a45cd43f11',
				price: 999.99,
				stock: 5,
				storage: '128GB',
			},
		],
	},
	{
		brand: 'Samsung',
		colors: [{ color: '#FF0000', color_name: 'แดง' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['แรม 6GB', 'จอ Super AMOLED', 'กล้อง 64MP'],
		id: 'f1e2d3b4-7a5c-4f5b-b9e1-a3d18c321bcd',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'Samsung Galaxy A72',
		price: 699.99,
		slug: 'samsung-galaxy-a72',
		variants: [
			{
				color: '#FF0000',
				color_name: 'แดง',
				id: 'l2m3n4o5-fe26-4e08-9a2b-28a61dh14k23',
				price: 699.99,
				stock: 12,
				storage: '256GB',
			},
		],
	},
	{
		brand: 'Xiaomi',
		colors: [{ color: '#00FF00', color_name: 'เขียว' }],
		created_at: new Date().toISOString(),
		description: {
			type: 'doc',
			content: [
				{
					type: 'paragraph',
					content: [
						{
							type: 'text',
							text: 'คำอธิบายตัวอย่าง',
						},
					],
				},
			],
		},
		features: ['แรม 8GB', 'แบตเตอรี่ 5000mAh', 'หน้าจอ 90Hz'],
		id: 'e5a1d2be-5b60-4f3e-b8d1-a3c98c123abf',
		images: ['https://ui.shadcn.com/placeholder.svg'],
		name: 'Xiaomi Redmi Note 10',
		price: 299.99,
		slug: 'xiaomi-redmi-note-10',
		variants: [
			{
				color: '#00FF00',
				color_name: 'เขียว',
				id: 'k4l5m6n7-fe26-4e08-9c2d-29a72ef15l24',
				price: 299.99,
				stock: 15,
				storage: '128GB',
			},
		],
	},
];
